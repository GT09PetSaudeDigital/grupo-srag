...
def transform_srag_dataframe(df: pd.DataFrame, year: int) -> pd.DataFrame:
...
def write_year_parquet(df: pd.DataFrame, base_dir: Path, year: int) -> Path:
    output_path = base_dir / "srag" / f"ano={year}" / "srag.parquet"
...
def ingest_year(
    ...
    output_path = parquet_root / "srag" / f"ano={year}" / "srag.parquet"
    ...
    parquet_path = write_year_parquet(processed_df, parquet_root, year)

    report = build_quality_report(raw_df, processed_df, year)
    write_quality_report(report, quality_root / f"quality_{year}.json")
    return parquet_path
